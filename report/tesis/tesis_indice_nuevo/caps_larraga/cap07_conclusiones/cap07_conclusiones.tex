...
El FoM promedio de $0{,}317$ (Tabla~\ref{tab:metrics_all_periods}) se sitúa en la banda intermedia del espectro multi-sitio de \textcite{pontius2008comparing}: por encima del subconjunto con FoM inferior a $0{,}15$ y por debajo del único caso que supera $0{,}50$. El Kappa de $0{,}445$ corresponde a un acuerdo moderado en la escala de \textcite{LandisKoch1977}, que es su marco de lectura porque aquel estudio no reporta Kappa. El IoU de $0{,}633$ se mide sobre el estado urbano final, a diferencia del FoM, que es una intersección sobre unión restringida a las celdas de cambio. El umbral $\theta = 0{,}75$ y el peso de vecindad $\alpha = 0{,}50$ son la configuración usada en las cinco ventanas; ensayos preliminares con umbrales menores produjeron sobre-predicción masiva, con un FoM que no es directamente comparable porque se midió bajo otro protocolo de conteo (Capítulo~\ref{ch:resultados-analisis}).
...
\textbf{R3, replicabilidad y apertura: cumplido en el procesamiento.} Todo el tratamiento de los datos se ejecuta en Python con bibliotecas de código abierto sobre CPU estándar, en un cómputo del orden de tres horas, con calibración sistemática documentada y sin software propietario ni hardware especializado. La adquisición de las imágenes queda fuera de esa apertura, porque depende del visor de Google Earth.
...
El pipeline completo (clasificación + WoE + simulación + validación) se ejecuta en aproximadamente 3 horas en CPU estándar, sin requerir GPU. Los enfoques de aprendizaje profundo en teledetección suelen exigir conjuntos anotados grandes \parencite{Ma2019} y, en la práctica, hardware gráfico; esta tesis no midió horas de GPU de terceros. La Tabla~\ref{tab:approach_characteristics} resume las características operativas del enfoque adoptado.
...
A corto plazo destacan cuatro líneas. La primera es la incorporación de \textbf{variables adicionales}: distancia a vialidades principales, distancia a centros de empleo y pendiente topográfica mediante análisis WoE multi-variable. La segunda es la \textbf{optimización metaheurística}, esto es, explorar metaheurísticas alternativas al grid exhaustivo (p.\,ej.\ optimización por enjambre de partículas o procedimientos bayesianos de búsqueda) para la calibración de parámetros, comparando su desempeño con el procedimiento empleado en esta investigación. La tercera es la \textbf{validación en más ciudades}: aplicar la metodología a Aguascalientes, León y San Luis Potosí para evaluar la transferibilidad geográfica. La cuarta es la \textbf{reconstrucción de la serie sobre imagen calibrada}: rehacer los mapas binarios a partir del archivo Landsat, que cubre el período completo con georreferencia y banda infrarroja, y contrastar la clasificación RGB actual contra una basada en NDVI. Ese contraste permitiría estimar qué parte del error de clasificación proviene de la fuente y no del clasificador, que es la pregunta que la serie de capturas deja abierta (Sección~\ref{sec:preprocesamiento}).
