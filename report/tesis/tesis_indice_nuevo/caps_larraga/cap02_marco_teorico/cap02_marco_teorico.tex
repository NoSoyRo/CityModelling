...
El agrupamiento K-Means debe su nombre y su criterio a \textcite{MacQueen1967}: es un método no supervisado que particiona un conjunto de observaciones en $k$ grupos buscando una varianza intraclase baja, medida como la suma de distancias cuadradas de cada punto al centroide de su grupo. La variante que se emplea aquí no es el procedimiento secuencial de una pasada que describe MacQueen, sino el esquema iterativo por lotes que él mismo atribuye a Forgy y a Jennrich, equivalente al procedimiento de cuantización óptima que \textcite{Lloyd1982} había formulado en 1957 y que solo se publicó en 1982: asigna cada observación al centroide más próximo, recalcula los centroides como la media de los puntos asignados y repite hasta que las asignaciones se estabilizan. El resultado depende de la inicialización, por lo que en la práctica se ejecuta varias veces y se conserva la partición de menor inercia. Su utilidad en clasificación de imágenes está en que produce una partición del espacio de características sin necesidad de etiquetas previas; su límite es que esa partición carece de significado semántico, de modo que asociar cada grupo a una clase de cobertura requiere una decisión posterior del analista. El procedimiento completo figura en el Apéndice~\ref{ap:arquitectura} (Algoritmo~\ref{alg:kmeans}).
...
En los AC probabilísticos, la función de transición $\delta$ asigna probabilidades a cada posible nuevo estado, en lugar de determinar el siguiente estado de forma determinista. Esta extensión, presente en la línea de modelos urbanos que abre el autómata restringido de \textcite{White1993} y en marcos calibrados como SLEUTH \parencite{Clarke1998}, resulta adecuada para el modelado urbano, en el que la transición de no-urbano a urbano no está garantizada incluso cuando las condiciones locales son favorables.

Entre los marcos de autómatas celulares urbanos, el más difundido es el que hoy se conoce como SLEUTH, formulado como autómata auto-modificable por \textcite{ClarkeHoppen1997} y aplicado a la bahía de San Francisco y al corredor Washington-Baltimore por \textcite{Clarke1998}. El acrónimo, que resume las capas de entrada, queda definido explícitamente en la aplicación a Lisboa y Oporto \parencite{Silva2002}, donde el modelo se presenta como sucesor del \textit{Clarke Cellular Automaton Urban Growth Model}: \textit{slope} (pendiente), \textit{land use} (uso de suelo), \textit{exclusion} (zonas excluidas del crecimiento), \textit{urban} (mancha urbana histórica), \textit{transportation} (vialidades) e \textit{hillshade} (sombreado del terreno). Su comportamiento queda gobernado por cinco coeficientes de crecimiento (\emph{diffusion}, \emph{breed}, \emph{spread}, \emph{slope resistance} y \emph{road gravity}) \parencite{ClarkeHoppen1997} que se ajustan mediante búsqueda sobre un espacio discreto de combinaciones, evaluando cada una con simulaciones Monte Carlo. Estas definiciones son las que el Capítulo~\ref{ch:estado-del-arte} da por sentadas al discutir las dificultades prácticas de esa calibración.
...
\begin{figure}[htbp]
	...
	\caption{Vecindad de Moore ($8$ celdas adyacentes) frente a Von~Neumann ($4$ ortogonales). La de Von~Neumann propaga el crecimiento solo en las direcciones cardinales; la de Moore admite además frentes diagonales. \textit{Imagen elaborada con IA generativa, con base en las reglas bidimensionales de \textcite{PackardWolfram1985}.}}
...
El IV es no negativo y crece cuando las dos distribuciones se separan. La escala convencional de interpretación, tomada de esa misma práctica \parencite{Siddiqi2006}, lo divide en cinco tramos: por debajo de $0{,}02$ la variable no tiene valor predictivo; entre $0{,}02$ y $0{,}10$ es débil; entre $0{,}10$ y $0{,}30$, medio; entre $0{,}30$ y $0{,}50$, fuerte; y por encima de $0{,}50$ la propia fuente lo califica de sospechoso, porque en su ámbito de origen un valor tan alto suele delatar que la variable ya contiene la respuesta. Esa última advertencia se formuló para el crédito, donde el predictor y el evento provienen de registros distintos; aquí los predictores se derivan del mismo mapa binario que el objetivo, y que la urbanización ocurra junto a lo ya urbano es el mecanismo que se quiere capturar, no un artefacto del muestreo.
...
La \textbf{matriz de confusión} $\mathbf{C} \in \mathbb{N}^{2 \times 2}$ cuantifica la concordancia píxel a píxel entre predicción y observación, y es el instrumento base sobre el que se construye la evaluación de exactitud en los estudios de cambio de cobertura \parencite{PontiusMillones2011}:
...
El rango es $\kappa \in [-1, 1]$. Conforme a la escala de interpretación de uso común propuesta por \textcite{LandisKoch1977}, $0{,}81$--$1{,}00$ indica acuerdo casi perfecto; $0{,}61$--$0{,}80$, acuerdo sustancial; $0{,}41$--$0{,}60$, acuerdo moderado; $0{,}21$--$0{,}40$, acuerdo justo; $0{,}00$--$0{,}20$, acuerdo leve; y $\kappa < 0$, acuerdo pobre. Los propios autores advierten que estas divisiones son convencionales y se ofrecen como puntos de referencia para la discusión, no como umbrales de validez. La ventaja del coeficiente $\kappa$ frente a la exactitud es que descuenta el acuerdo esperado por azar, haciendo la comparación más justa cuando las clases están desbalanceadas. Su limitación, identificada por \textcite{PontiusMillones2011}, es que mezcla en un solo número dos fuentes de error metodológicamente distintas: el desacuerdo de cantidad y el de asignación.
...
El denominador incluye exactamente $A + B + C$, que son las celdas donde el cambio ocurre, se predice o ambas. Las celdas donde se observó y se predijo permanencia no entran en el cálculo; las celdas estables que el modelo predice como cambio sí, porque son la comisión $A$.\footnote{La notación se adapta aquí al caso binario. \textcite{pontius2008comparing} reservan $A$ para la omisión y $D$ para la comisión, y añaden un término $C$ para el cambio predicho hacia la categoría de destino equivocada. Con una sola clase de destino ese término es nulo, de modo que las dos expresiones coinciden numéricamente, pero las letras $A$ y $C$ no tienen aquí el mismo significado que en el artículo original.}
