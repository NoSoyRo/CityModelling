...
una literatura abundante pero metodológicamente desigual. Este capítulo no
cataloga trabajos por autor sino que identifica los cuatro problemas
...
del modelo WoE-AC propuesto. La discusión cierra con un mapa de la literatura
...
El modelado de la dinámica urbana mediante autómatas celulares se consolidó con
\textcite{White1997}, que mostró que reglas locales basadas en distancia y
vecindad podían reproducir patrones metropolitanos realistas. La generalización
del enfoque vino con SLEUTH \parencite{Clarke1998,Silva2002}, que durante dos
décadas funcionó como referencia y se aplicó en decenas de ciudades.
Paralelamente, la tradición LUCC (Land Use and Cover Change) introdujo modelos
basados en mapas de idoneidad estadística: familias como CLUE-S, Dinamica EGO,
CA-Markov y, más recientemente, FLUS, GeoSOS y MOLUSCE, ofrecieron
arquitecturas alternativas al CA puro en las que la función de transición
emerge de regresiones logísticas o pesos bayesianos sobre datos históricos.

Una corriente posterior sustituyó la regresión por modelos de aprendizaje
automático e híbridos metaheurísticos. \textcite{almeida2008stochastic} acopló
una red neuronal feedforward a un CA estocástico para Piracicaba, Brasil; años
más tarde \textcite{Wang2021} combinó un autómata celular con un algoritmo
genético para optimizar la forma urbana de Wuhan, mientras
...
ganancias cuantitativas, pero sustituyen pesos legibles por activaciones
internas o cromosomas que no admiten una lectura geográfica directa.

La tensión central del campo no es entonces entre AC y aprendizaje automático,
sino entre dos objetivos que parecen contrapuestos: maximizar precisión
...
final, que reducen el espacio iterativamente; aun así, una calibración completa
...
entre combinaciones equifinales termina dependiendo del operador.
...
importante: cada configuración candidata produce mapas de idoneidad cuyos pesos
por variable son directamente auditables, eliminando la ambigüedad asociada a
la equifinalidad.

\subsection{Validación: sobreajuste temporal y métricas no comparables}

\textcite{pontius2008comparing} analizó trece aplicaciones independientes de
nueve modelos de cambio de uso de suelo y propuso el Figure of Merit (FoM) como
métrica estándar comparable entre estudios. Su hallazgo es incómodo: seis de
las trece aplicaciones obtienen FoM inferior al $15\%$ y solo una supera el
$50\%$. La dispersión no se explica por la sofisticación del modelo sino por la
magnitud del cambio neto observado: el FoM crece monotónicamente con el
porcentaje de celdas que efectivamente cambian de estado.

Esto tiene dos implicaciones que la literatura rara vez confronta. Primero,
\textbf{validar en una sola ventana temporal es estadísticamente insuficiente}:
si el FoM depende de la magnitud del cambio, una validación única no permite
distinguir entre un modelo robusto y uno ajustado a una fase específica del
crecimiento. \textcite{Sante2010} y \textcite{Aburas2016} confirman, en sus
respectivas revisiones sistemáticas, que la validación en un único período es
la norma, no la excepción, en la literatura de Autómatas Celulares. Segundo, \textbf{reportar
...
Accuracy de $0.90$ sin capturar ninguna dinámica.

La consecuencia metodológica es directa: la validación del WoE-AC se realiza en
...
distinguir mecanismos espaciales reales de sobreajuste.
